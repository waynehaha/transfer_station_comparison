...
function displayPriceSet(provider) {
  return window.ProviderCore.displayPriceSet(provider, priceDisplayMode);
}

function displayPriceLabels() {
  return window.ProviderCore.displayPriceLabels(priceDisplayMode);
}

function displayPricePrefix() {
  return window.ProviderCore.displayPricePrefix(priceDisplayMode);
}
...
function exportProviders() {
  ...
  const payload = buildExportPayload(providers);
...
async function mergeProvidersFromFile(file) {
  ...
  const rawText = await file.text();
  const importedProviders = parseImportedProviders(rawText);
  const { merged, stats } = mergeImportedProviders(providers, importedProviders);
  providers = merged;
  clearEditState({ restoreOfficial: false });
  localStorage.setItem(STORAGE_KEY, JSON.stringify(providers));
  await persistProviders();
